...
const Video: React.FC<VideoProps> = (props) => {
    ...
    return (
        <>
            <div key={props._id.toString()} id={'video'} className={'w-72 flex flex-col mb-5'}>
                <Link id={'thumbnail'} href={`/watch/${props._id}`}
                      className={'relative text-xs p-3 w-full cursor-pointer'}>
                    ...
                    <div id={'video-length'}
                         className={'absolute text-sm right-4 bottom-4 bg-zinc-950 px-1 bg-opacity-80 rounded-md'}>
                        {formatDuration(props.duration)}
                    </div>
                </Link>
                <div id={'details'} className={'p-3 w-full flex flex-row justify-between items-start'}>
